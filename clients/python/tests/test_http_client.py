...
from mnemovela_client import MnemovelaHttpClient, AsyncMnemovelaHttpClient, MnemovelaError


class TestHttpClient(unittest.TestCase):
    def test_add_episode(self):
        with serve_http() as url:
            c = MnemovelaHttpClient(url)
    ...
    def test_unknown_method_raises(self):
        with serve_http() as url:
            c = MnemovelaHttpClient(url)
            with self.assertRaises(MnemovelaError) as ctx:
    ...
    def test_async_add_episode(self):
        async def run():
            with serve_http() as url:
                c = AsyncMnemovelaHttpClient(url)
